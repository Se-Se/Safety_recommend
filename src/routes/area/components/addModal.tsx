...
export default function AddModal(props) {
  const { add, update } = useIndexedDB(DBTableName.area);
  const getUseDb = () => {
  ...
  const templageFn = () => {
    return (
      <Form>
        ...
        <Form.Item
          label="包含系统"
          message={doSave ? (belongSelect.length !== 0 ? null : '请选择包含系统') : null}
          status={doSave ? (belongSelect.length !== 0 ? null : 'error') : null}
        >
          <SelectMultiple
            value={belongSelect}
            appearance="button"
        ...
        <Form.Item
          label="包含网络资产"
          message={doSave ? (propertyName.length !== 0 ? null : '请选择包含系统') : null}
          status={doSave ? (propertyName.length !== 0 ? null : 'error') : null}
        >
          <SelectMultiple
            value={propertyName}
            appearance="button"
